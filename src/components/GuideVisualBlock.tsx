    ...
    <section className="guide-visual-block mt-12 rounded-lg border border-[var(--border)] bg-[var(--background)] p-6 sm:p-8">
      <div>
        <div className="max-w-2xl">
          ...
          <h2 className="mt-4 font-serif text-3xl leading-tight text-[var(--navy)] sm:text-[2.35rem]">
            {title}
          </h2>
          {description ? (
            <p className="mt-4 leading-7 text-[var(--muted)]">{description}</p>
          ) : null}
          <div aria-hidden="true" className="mt-7 hidden max-w-md sm:block">
            <span className="guide-motion-line relative block h-px bg-[var(--gold)]/45">
        ...
        <div className="mt-8 grid auto-rows-fr gap-3 sm:grid-cols-2">
          {items.map((item) => (
            <article
              ...
              className="h-full rounded-lg border border-[var(--border)] bg-white p-5 sm:min-h-[10.5rem]"
            >
